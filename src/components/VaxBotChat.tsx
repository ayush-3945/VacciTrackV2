      ...
      <div className="fixed bottom-6 right-6 z-50">
        ...
        >
          <button
            ...
            className="flex items-center gap-2.5 px-4 py-2.5 rounded-full bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg transition-all duration-200 border border-emerald-500/30 group"
            aria-label="Open VaxBot AI Assistant"
          >
            <div className="relative">
              <Bot className="w-5 h-5" />
              <span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-emerald-300 rounded-full border-2 border-emerald-700" />
            </div>
            <span className="font-display font-medium text-xs pr-1 hidden sm:inline-block">
              {isOpen ? 'Close Assistant' : 'Ask VaxBot AI'}
            </span>
          </button>
        </motion.div>
      </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-20 right-4 sm:right-6 w-[calc(100vw-2rem)] sm:w-[420px] h-[560px] max-h-[82vh] bg-card border border-border shadow-2xl rounded-2xl z-50 flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="bg-secondary/80 border-b border-border px-4 py-3 flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="font-semibold text-sm text-foreground">VaxBot AI</h3>
                  ...
                  <p className="text-[11px] text-muted-foreground">NIS 2025 Clinical AI Assistant</p>
                </div>
              </div>
            ...
            <div className="p-2.5 bg-secondary/30 border-b border-border flex-shrink-0">
              <div className="flex items-center justify-between mb-1.5 px-1">
                <span className="text-[11px] font-medium text-muted-foreground flex items-center gap-1">
                  <HelpCircle className="w-3.5 h-3.5 text-emerald-500" /> FAQs
                </span>
              ...
              <div className="flex gap-1.5 overflow-x-auto pb-1 pt-0.5 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                {currentQuestions.map((q, idx) => (
                  <button
                    ...
                    className="px-2.5 py-1 rounded-md text-[11px] whitespace-nowrap font-normal bg-secondary hover:bg-secondary/80 text-muted-foreground hover:text-foreground border border-border transition-all text-left flex-shrink-0"
                  >
                    {idx + 1}. {q}
                  </button>
                ))}
              ...
              {messages.map(msg => (
                ...
                >
                  <div className="flex items-start gap-2 max-w-[88%]">
                    {msg.sender === 'bot' && (
                      <div className="w-6 h-6 rounded-md bg-secondary text-foreground flex items-center justify-center flex-shrink-0 mt-0.5 border border-border">
                    ...
                    <div
                      className={cn(
                        'p-3 rounded-xl text-xs leading-relaxed shadow-xs relative group',
                        msg.sender === 'user'
                          ? 'bg-emerald-600 text-white rounded-tr-xs'
                          : 'bg-secondary/60 text-foreground border border-border rounded-tl-xs'
                      )}
                    >
                      ...
                      {msg.sender === 'bot' && msg.actionType === 'certificate' && (
                        <div className="mt-2.5 pt-2 border-t border-border">
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-emerald-600 text-white text-xs font-medium hover:bg-emerald-500 transition-colors shadow-xs"
                          >
                      ...
                      {msg.sender === 'bot' && msg.id === 'welcome' && (
                        <div className="mt-2.5 pt-2 border-t border-border flex flex-wrap gap-2">
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-secondary hover:bg-secondary/80 text-foreground border border-border text-xs font-medium transition-all"
                          >
                            <FileText className="w-3.5 h-3.5 text-muted-foreground" /> Download Certificate
                          </button>
                          <button
                            onClick={() => {
                              setIsOpen(false);
                            ...
                            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-secondary hover:bg-secondary/80 text-foreground border border-border text-xs font-medium transition-all"
                          >
                            <Calendar className="w-3.5 h-3.5 text-muted-foreground" /> Full Schedule
                          </button>
                        </div>
                      )}
                      ...
                      {msg.sender === 'bot' && (
                        <button
                          ...
                          className="mt-2 text-[10px] text-muted-foreground hover:text-foreground inline-flex items-center gap-1 px-2 py-0.5 rounded bg-background border border-border transition-colors"
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2 pl-8">
                      {msg.suggestions.map((suggestion, sIdx) => (
                        <button
                          ...
                          className="px-2.5 py-1 rounded-md text-[11px] font-normal bg-secondary hover:bg-secondary/80 text-foreground border border-border transition-all text-left"
                        >
                          {suggestion}
                        </button>
                      ))}
            ...
            <div className="p-3 border-t border-border bg-card flex-shrink-0">
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 px-3.5 py-2 rounded-lg border border-border bg-secondary/50 text-foreground placeholder:text-muted-foreground text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500/30 focus:border-emerald-500"
                />
                <Button
                  onClick={() => handleSendMessage()}
                  disabled={!inputMessage.trim() || isTyping}
                  size="sm"
                  className="rounded-lg px-3 bg-emerald-600 hover:bg-emerald-500 text-white h-9 w-9 p-0 flex items-center justify-center flex-shrink-0"
                >
